...
class CCusto extends MY_Controller {
	...
	function index(){
		...
		$data['produtos'] = $this->MCategoria->findAllCategorias();
	...
	function buscarPorPeriodo(){
		...
		$data=  $this->MCusto->findCustosByPeriod($dataInicial, $dataFinal, $login_id);
	...
	function buscarCustoManutencao(){
		...
		$data['custos'] = $this->MCusto->findCustosByParameters($baseBusca, $login_id);
		print json_encode($data);
	}
	...
	function buscarCustosDia(){
		...
		$data['custos'] =  $this->MCusto->findCustosByData($dataAtual, $login_id);
		print json_encode($data);
	}

	/**
	 * Funcao que retorna a listagem dos custos do mes atual pelo usu�rio logado
	 * filtrando de acordo com o c�digo do produto passada por par�metro
	 * @param $produto_id
	 */
	function buscarCustosMesPelaCategoria(){
		...
		$data['custos'] =  $this->MCusto->findCustosByCategoryAndPeriod($dataInicio, $dataFinal, $produto_id, $login_id);
		print json_encode($data);
	}
...
/* 			if($countParcelas > 1){
	...
	private function buscaSomatoriaCustosMes(){
		$login_id = $this->input->get_post('login_id', TRUE) ? $this->input->get_post('login_id', TRUE) : $_SESSION['login_id'];
		return $this->MCusto->buscarSomaCustosPeriodo("01/" . date("m") ."/" . date("Y"), date("t/m/Y"), $login_id);
	}

	private function buscaSomatoriaEntradasMes(){
		$login_id = $this->input->get_post('login_id', TRUE) ? $this->input->get_post('login_id', TRUE) : $_SESSION['login_id'];
		return $this->MEntrada->buscarSomaEntradasPeriodo("01/" . date("m") ."/" . date("Y"), date("t/m/Y"), $login_id);
	}
	...
	function geraGraficoCustosCategoria(){
		...
		$gastosCategoriaMes = $this->MCusto->buscarGastosCategoriaPeriodo("01/" . date("m") ."/" . date("Y"), date("t/m/Y"), $login_id);
		...
		foreach ($gastosCategoriaMes as $g){
			$xml.= "<set label='".$g['descricao']."' value='".$g['total']."' link='javascript:updateGridGastosCatMes(".$g['produto_id'].",\"".$g['descricao']."\","."\"01/" . date("m") ."/" . date("Y")."\",\"".date("t/m/Y")."\");'/>";
		}
		$xml.= "</chart>";
		print $xml;
	}
	...
	function gerarGraficoCustosCategoriaMensal($dataInicial = null, $dataFinal = null){
		...
		$categorias = $this->MCusto->buscarCategoriasMes($dataInicial, $dataFinal, $login_id);
		$mesesTotais = $this->MCusto->buscarGastosCategoriaTotaisMensal($dataInicial, $dataFinal, $login_id);
		...
		foreach ($produtos as $c){
			...
			$gastosCategoriaMes = $this->MCusto->buscarGastosCategoriaMensal($dataInicial, $dataFinal, $login_id, $c['produto_id']);
	...
	function gerarGraficoCustosEntradasMensal($dataInicial = null, $dataFinal = null){
		...
		$custosEntradasMeses = $this->MCusto->buscarCustosEntradaAgrupadosMes("01/".$dataInicial[1]."/".$dataInicial[2], "31/".$dataFinal[1]."/".$dataFinal[2], $login_id);
	...
	function geraGraficoCustosCategoriaTotaisPeriodo($dataInicial = null, $dataFinal = null){
		...
		$gastosCategoriaMes = $this->MCusto->buscarGastosCategoriaTotaisPeriodo($dataInicial, $dataFinal, $login_id);
		...
		foreach ($gastosCategoriaMes as $g){
			$xml.= "<set label='".$g['produto']."' value='".$g['total']."'/>";
		}
		$xml.= "</chart>";
		print $xml;
	}
	...
	function geraGraficoEntradasCustosPeriodo($dataInicial = null, $dataFinal = null){
		...
		$somaCustosMes = $this->MCusto->buscarSomaCustosPeriodo($dataInicial, $dataFinal, $login_id);
		$somaEntradasMes = $this->MEntrada->buscarSomaEntradasPeriodo($dataInicial, $dataFinal, $login_id);
		print "<chart numberPrefix='R$ ' showValues='1' rotateValues='0' placeValuesInside='1' decimalSeparator=',' thousandSeparator='.' decimals='2' formatNumberScale='0'><set label='Entradas' value='".$somaEntradasMes."' /><set label='Custos' value='".$somaCustosMes."' /></chart>";
	}
}
